...
#define TAG 6543
      ...
      caffe::mpi::sendrecv(one_data_, datum_size, dest_, TAG,
          this->data_.get() + pnetcdf_offset, datum_size, source_, TAG, comm_);
      if (this->output_labels_) {
        memcpy(one_label_, this->label_.get()+row, sizeof(int));
        caffe::mpi::sendrecv(one_label_, 1, dest_, TAG,
            this->label_.get()+row, 1, source_, TAG, comm_);
      }
    }
