...
MPI_layerwise_async_rotate_CPU<Dtype>::MPI_layerwise_async_rotate_CPU(shared_ptr<Solver<Dtype> > root_solver,
 ...
 : CPUParams<Dtype>(root_solver),
  ...
  stopping(0),
  background_thread_ (std::thread(&MPI_layerwise_async_rotate_CPU::background_task, this, num_layers_))
{
  std::clog << "Initializing mpi_layerwise_async_rotate_cpu with effective rgroup bits = " << rgroup_bits_ << std::endl;
  {
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::on_start() {
  DLOG(INFO) << "on_start()";
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::on_gradients_ready(int param_id) {
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::background_task(const int num_learnable_layers) {
...
*/
        ...
        const int remote_node = ((buddies[0] == background_comm_rank)
                                 ...
                                 : buddies[0]);
        //
        // Exchange diffs first, then data
        //
...
*/
        ...
        // signal other thread that we're done copying and averaging
      ...
      // rotate which layer we're communicating with
      current_stage++;
    ...
    // we've already applied the gradient in the background
    current_layer = (num_learnable_layers - 1); 
    ...
    if (!stopping) {
      ...
      // remapping would go here if constant by-layer
/* 
      std::clog << "Node [" << background_comm_rank << "] "
...
template<typename Dtype>
int MPI_layerwise_async_rotate_CPU<Dtype>::on_apply(int param_id) {
/*
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::shuffle_vector(int *array_ptr, const int num_elements) {
  if (num_elements > 2) {
...
template<typename Dtype>
MPI_layerwise_async_rotate_CPU<Dtype>::~MPI_layerwise_async_rotate_CPU() {
  stopping =1;
  while (background_running) {};
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::mpi_avg_2(Dtype * real_buffer,
                                  Dtype * temp_buffer,
                                  const size_t pcount,
                                  const int root_node,
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::on_gradients_ready() {
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::on_post_apply() {
}
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::Run() {
  LOG(INFO)<< "Starting Optimization - MPI_layerwise_async_rotate_CPU";
...
template<typename Dtype>
void MPI_layerwise_async_rotate_CPU<Dtype>::Step(int iters) {
...
INSTANTIATE_CLASS(MPI_layerwise_async_rotate_CPU);
