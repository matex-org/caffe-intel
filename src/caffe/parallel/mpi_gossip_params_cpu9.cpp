...
void MPIGossipParamsCPU9<Dtype>::next() {
  if (cube_) {
    if (rotate_) {
      next_cube_rotate();
    }
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::next_cube() {
  if (hci_ > logp_) {
    hci_ = 0;
  }
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::next_cube_rotate() {
  if (hci_ > logp_) {
    hci_ = 0;
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::next_diffuse() {
  if (hci_ > logp_) {
    hci_ = 0;
  }
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::next_diffuse_rotate() {
  if (hci_ > logp_) {
    hci_ = 0;
    mci_ = (mci_+1)%comm_size_;
    comm_rank_ = caffe::mpi::comm_rank(comms_[mci_]);
...
template<typename Dtype>
MPIGossipParamsCPU9<Dtype>::MPIGossipParamsCPU9(
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::on_start() {
  DLOG(INFO) << "on_start()";
  CPUTimer timer;
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::make_progress() {
  CPUTimer timer;
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::on_forward(int param_id) {
  DLOG(INFO) << "on_forward(param_id)";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::on_gradients_ready(int param_id) {
  DLOG(INFO) << "on_gradients_ready(param_id)";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::on_gradients_ready() {
  DLOG(INFO) << "on_gradients_ready()";
  make_progress();
}

template<typename Dtype>
int MPIGossipParamsCPU9<Dtype>::on_apply(int param_id) {
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::on_update() {
  DLOG(INFO) << "on_update()";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::Run() {
...
template<typename Dtype>
void MPIGossipParamsCPU9<Dtype>::Step(int iters) {
...
INSTANTIATE_CLASS(MPIGossipParamsCPU9);
