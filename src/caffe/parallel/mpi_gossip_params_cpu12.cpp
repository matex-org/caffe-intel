...
void MPIGossipParamsCPU12<Dtype>::next() {
  if (cube_) {
    if (rotate_) {
      next_cube_rotate();
    }
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::next_cube() {
  if (hci_ > logp_) {
    hci_ = 0;
  }
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::next_cube_rotate() {
  if (hci_ > logp_) {
    hci_ = 0;
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::next_diffuse() {
  if (hci_ > logp_) {
    hci_ = 0;
  }
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::next_diffuse_rotate() {
  if (hci_ > logp_) {
    hci_ = 0;
    mci_ = (mci_+1)%comm_size_;
    comm_rank_ = caffe::mpi::comm_rank(comms_[mci_]);
...
template<typename Dtype>
MPIGossipParamsCPU12<Dtype>::MPIGossipParamsCPU12(
...
template<typename Dtype>
bool MPIGossipParamsCPU12<Dtype>::param_needs_reduce(int param_id) {
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::on_start() {
  DLOG(INFO) << "on_start()";
  CPUTimer timer;
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::make_progress() {
  CPUTimer timer;
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::on_forward(int param_id) {
  DLOG(INFO) << "on_forward(param_id)";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::on_gradients_ready(int param_id) {
  DLOG(INFO) << "on_gradients_ready(param_id)";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::on_gradients_ready() {
  DLOG(INFO) << "on_gradients_ready()";
  make_progress();
}

template<typename Dtype>
int MPIGossipParamsCPU12<Dtype>::on_apply(int param_id) {
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::on_update() {
  DLOG(INFO) << "on_update()";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::Run() {
...
template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::do_allreduce() {
  CPUTimer timer;

  LOG(INFO)<< "do_allreduce()";

  timer.Start();
  for (size_t i=0; i<params_.size(); ++i) {
    if (!param_needs_reduce(i)) continue;
    if (CAN_USE_PRV_DATA(params_[i])) {
      caffe::mpi::allreduce(params_[i]->mutable_prv_data(), params_[i]->prv_data_count(), MPI_SUM, comms_[0]);
      caffe_scal(params_[i]->prv_data_count(), Dtype(0.5), params_[i]->mutable_prv_data());
    }
    else {
      caffe::mpi::allreduce(params_[i]->mutable_cpu_data(), params_[i]->count(), MPI_SUM, comms_[0]);
      caffe_scal(params_[i]->count(), Dtype(0.5), params_[i]->mutable_cpu_data());
    }
  }
  for (size_t i=0; i<sgdsolver_->history().size(); ++i) {
    if (!param_needs_reduce(i)) continue;
    if (CAN_USE_PRV_DATA(sgdsolver_->history()[i])) {
      caffe::mpi::allreduce(sgdsolver_->history()[i]->mutable_prv_data(), sgdsolver_->history()[i]->prv_data_count(), MPI_SUM, comms_[0]);
      caffe_scal(sgdsolver_->history()[i]->prv_data_count(), Dtype(0.5), sgdsolver_->history()[i]->mutable_prv_data());
    }
    else {
      caffe::mpi::allreduce(sgdsolver_->history()[i]->mutable_cpu_data(), sgdsolver_->history()[i]->count(), MPI_SUM, comms_[0]);
      caffe_scal(sgdsolver_->history()[i]->count(), Dtype(0.5), sgdsolver_->history()[i]->mutable_cpu_data());
    }
  }
  timer.Stop();
  stats_sample_value(&stats_allreduce_, timer.MilliSeconds());
  LOG(INFO) << "time allreduce " << stats_allreduce_._mean
    << " += " << stats_stddev(&stats_allreduce_)
    << " min " << stats_allreduce_._min
    << " max " << stats_allreduce_._max;
}

template<typename Dtype>
void MPIGossipParamsCPU12<Dtype>::Step(int iters) {
...
INSTANTIATE_CLASS(MPIGossipParamsCPU12);
