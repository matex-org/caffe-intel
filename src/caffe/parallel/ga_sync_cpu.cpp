...
GASyncCPU<Dtype>::GASyncCPU(shared_ptr<Solver<Dtype> > root_solver)
    : comm_rank_(),
      comm_size_(),
      solver_(),
      sgdsolver_(),
...
template<typename Dtype>
GASyncCPU<Dtype>::~GASyncCPU() {
}

template<typename Dtype>
bool GASyncCPU<Dtype>::param_needs_reduce(int param_id) {
...
template<typename Dtype>
void GASyncCPU<Dtype>::on_gradients_ready(int param_id) {
  DLOG(INFO) << "on_gradients_ready(" << param_id << ")";
}

template<typename Dtype>
void GASyncCPU<Dtype>::Run() {
...
template<typename Dtype>
void GASyncCPU<Dtype>::Step(int iters) {
