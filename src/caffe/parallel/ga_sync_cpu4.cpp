...
GASyncCPU4<Dtype>::GASyncCPU4(shared_ptr<Solver<Dtype> > root_solver)
    : comm_rank_(),
      comm_size_(),
      solver_(),
      sgdsolver_(),
...
template<typename Dtype>
GASyncCPU4<Dtype>::~GASyncCPU4() {
}

template<typename Dtype>
bool GASyncCPU4<Dtype>::param_needs_reduce(int param_id) {
...
template<typename Dtype>
void GASyncCPU4<Dtype>::on_start() {
  DLOG(INFO) << "on_start()";
}

template<typename Dtype>
void GASyncCPU4<Dtype>::on_forward(int param_id) {
  int victim = rand() % comm_size_;
  DLOG(INFO) << "on_forward(" << param_id << ") victim=" << victim;

  int prefetch_id = -1;
  if (param_id == params_.size()-1) {
    prefetch_id = 0;
  }
  else {
    prefetch_id = param_id + 1;
  }

  /* prefetch data and history of random victim */
  ARMCI_NbGet(data_pointers_[prefetch_id][victim],
      data_recv_[prefetch_id],
      sizeof(Dtype)*params_[prefetch_id]->count(),
      victim,
      &data_hdl_[prefetch_id]);
  ARMCI_NbGet(hist_pointers_[prefetch_id][victim],
      hist_recv_[prefetch_id],
      sizeof(Dtype)*params_[prefetch_id]->count(),
      victim,
      &hist_hdl_[prefetch_id]);
...
template<typename Dtype>
void GASyncCPU4<Dtype>::on_gradients_ready() {
  DLOG(INFO) << "on_gradients_ready()";
  first_time_ = false;
}

template<typename Dtype>
void GASyncCPU4<Dtype>::on_gradients_ready(int param_id) {
  DLOG(INFO) << "on_gradients_ready(" << param_id << ")";
}

template<typename Dtype>
void GASyncCPU4<Dtype>::Run() {
...
template<typename Dtype>
void GASyncCPU4<Dtype>::Step(int iters) {
